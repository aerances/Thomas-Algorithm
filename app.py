...
def check_diagonal_dominance(a, b, c):
    """
    Validates if the tridiagonal matrix system is diagonally dominant[cite: 298].
    ...
    for i in range(n):
        ...
        if diag_val < (sub_val + super_val):
            violations.append(f"Row {i+1} (|{b[i]}| < |{a[i]}| + |{c[i]}|)")
            
    if violations:
        msg = "Warning: The system is not strictly diagonally dominant at: " + ", ".join(violations) + ". The algorithm may become unstable or fail due to division by zero[cite: 296, 298]."
        return False, msg
    return True, "Success: Matrix passes strict diagonal dominance checks[cite: 298, 300]."

def thomas_algorithm_with_steps(a, b, c, r):
    """
    Executes the Thomas Algorithm for a tridiagonal matrix system Mx = r[cite: 119, 120].
    Tracks explicit step-by-step mathematical computations for Stage 1 and Stage 2[cite: 122].
    """
    n = len(b)
    gamma = [0.0] * n
    rho = [0.0] * n
    ...
    # --- STAGE 1: Forward Elimination / Downward Sweep [cite: 122, 123] ---
    steps.append({"type": "header", "text": "Stage 1: Forward Sweep (Computing Gamma and Rho vectors) [cite: 122]"})
    
    if b[0] == 0.0:
        raise ValueError("Calculation Halt: Diagonal element b_1 is exactly zero[cite: 296]. Cannot initialize forward substitution sweep.")
        
    gamma[0] = c[0] / b[0]
    rho[0] = r[0] / b[0]
    
    # Using .format() to safely keep LaTeX curly brackets intact
    ...
    for i in range(1, n - 1):
        ...
        if denom == 0.0:
            raise ValueError("Halt: Division by zero encountered at diagonal position {}. The system is mathematically unstable without pivoting[cite: 296, 301].".format(i+1))
    ...
    if denom_n == 0.0:
        raise ValueError("Halt: Division by zero encountered at the final row diagonal vector transition[cite: 296]. System is unstable.")
    ...
    # --- STAGE 2: Backward Substitution / Upward Sweep [cite: 122, 259] ---
    steps.append({"type": "header", "text": "Stage 2: Backward Substitution (Upward Sweep) [cite: 122]"})
